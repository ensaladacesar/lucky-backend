...
const createUserPoints = (request, response) => {
  ...
  pool.query('INSERT INTO points (quantity, user_id) VALUES (20, $1)', [user_id], (error, results) => {
    if (error) {
      response.status(400).send(error);
    }
    else{
...
const createUserCredit = (request, response) => {
  ...
  pool.query('INSERT INTO credit (quantity, user_id) VALUES (20, $1)', [user_id], (error, results) => {
    if (error) {
      response.status(400).send(error);
    }
    else{
